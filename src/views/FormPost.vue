<template>
  <div class="max-w-3xl mx-auto p-10">
    <h1 class="text-2xl font-semibold mb-2">
      {{ $t(isEdit ? 'FORM_POST.EDIT_POST' : 'FORM_POST.NEW_POST') }}
    </h1>
    <p class="text-gray-500 mb-6">{{ $t(isEdit ? 'FORM_POST.EDIT' : 'FORM_POST.NEW') }}</p>
    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
      ...
      <!-- ACTIONS -->
      <div class="flex justify-between items-center pt-4 border-t-gray-400 border-t">
        ...
        >
          <span v-if="!saving">{{ $t('BUTTON.PUBLIC') }}</span>
...
const { title, body, tagsInput, saving, errors, isValid, handleSubmit, goBack, isEdit } =
  usePostForm()
</script>
